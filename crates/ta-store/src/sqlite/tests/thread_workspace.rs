...
fn seed_durable_turns(store: &mut SqliteStore, session_id: &SessionId, run_id: &RunId) {
...
#[test]
fn thread_workspace_sqlite_restores_projection_and_rejects_atomic_failures() {
    let path = test_db_path("thread-workspace");
    let mut store = SqliteStore::open(&path).expect("store should open");
    let session = session("session-thread-sqlite");
    store.save_session(session.clone()).expect("session");
    for (event, timestamp) in [
        ...
        store
            .append_thread_workspace_event(&session.id, timestamp, event)
            .expect("event appends");
    }
    let before = store
        .thread_workspace(&session.id)
        .expect("workspace")
        .expect("record");
    let error = store
    ...
    assert_eq!(
        reopened.thread_workspace(&session.id).expect("workspace"),
        Some(before)
    );
...
#[test]
fn thread_workspace_sqlite_validates_durable_pins_and_restores_atomic_projection() {
    let path = test_db_path("thread-workspace-pins");
    let mut store = SqliteStore::open(&path).expect("store should open");
    let session_a = session("session-thread-sqlite-pin-a");
    let session_b = session("session-thread-sqlite-pin-b");
    let run_a = RunId::new("run-thread-sqlite-pin-a").expect("run id");
    let run_b = RunId::new("run-thread-sqlite-pin-b").expect("run id");
    ...
    for (index, event) in [
        ...
        ThreadWorkspaceEvent::PinAdded {
            pin: ThreadWorkspacePin {
                run_id: RunId::new("run-thread-sqlite-wrong").expect("run id"),
                cursor: ActivityCursor { sequence: 1 },
            },
        },
    ]
    ...
    assert!(expected.pins.is_empty());
    drop(store);
    let reopened = SqliteStore::open(&path).expect("store should reopen");
    assert_eq!(
        reopened.thread_workspace(&session_a.id).expect("workspace"),
        Some(expected)
    );
    let _ = std::fs::remove_file(path);
}
