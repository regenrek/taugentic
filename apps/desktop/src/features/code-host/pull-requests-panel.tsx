...
export function PullRequestsPanel({ codeHost, openUrl }: { codeHost: WorkbenchCodeHostState; openUrl(url: string): void }) {
  ...
  return <div testId="pull-requests-panel" style={{ display: "flex", flexDirection: "column", width: "100%", height: "100%", minHeight: 0, backgroundColor: palette.canvas }}>
    <div style={{ display: "flex", alignItems: "center", gap: 7, minHeight: 42, paddingLeft: 10, paddingRight: 8, borderBottomWidth: 1, borderColor: palette.border }}>
      ...
      <div testId="refresh-code-host" tabIndex={0} onClick={codeHost.refresh} style={button()}><text style={{ fontSize: 9 }}>Refresh</text></div>
      <div testId="toggle-code-host-connection" tabIndex={0} onClick={() => setShowConnection((current) => !current)} style={button()}><text style={{ fontSize: 9 }}>Accounts</text></div>
      <div testId="toggle-pull-request-create" tabIndex={codeHost.selectedAccount ? 0 : -1} onClick={() => { if (codeHost.selectedAccount) setShowCreation((current) => !current) }} style={button(Boolean(codeHost.selectedAccount))}><text style={{ fontSize: 9 }}>New PR</text></div>
    </div>
    ...
    {!!codeHost.accounts.length && <div style={{ display: "flex", flexWrap: "wrap", gap: 6, padding: 8, borderBottomWidth: 1, borderColor: palette.border }}>
      ...
      {codeHost.accounts.map((account) => <Fragment key={account.id}><div testId={`select-code-host-account-${account.id}`} tabIndex={0} onClick={() => codeHost.selectAccount(account)} style={button(true, codeHost.selectedAccount?.id === account.id)}><text style={{ fontSize: 9 }}>{account.displayName}</text></div></Fragment>)}
    </div>}
    {!!codeHost.selectedAccount && <div style={{ display: "flex", flexWrap: "wrap", gap: 6, padding: 8, borderBottomWidth: 1, borderColor: palette.border }}>
      ...
      {codeHost.remotes.map((remote) => <Fragment key={remote.remoteName}><div testId={`select-code-host-remote-${remote.remoteName}`} tabIndex={0} onClick={() => codeHost.selectRemote(remote)} style={button(true, codeHost.selectedRemote?.remoteName === remote.remoteName)}><text style={{ fontSize: 9 }}>{remote.remoteName} · {remote.repository.owner}/{remote.repository.name}</text></div></Fragment>)}
    </div>}
...
function AccountConnections({ codeHost }: { codeHost: WorkbenchCodeHostState }) {
  return <div testId="code-host-accounts" style={{ display: "flex", flexDirection: "column", gap: 8, padding: 10, backgroundColor: palette.panel, borderBottomWidth: 1, borderColor: palette.border }}>
    ...
    {codeHost.accounts.map((account) => <Fragment key={account.id}><div style={{ display: "flex", alignItems: "center", gap: 8 }}>
      ...
      <div testId={`disconnect-code-host-account-${account.id}`} tabIndex={codeHost.accountBusy ? -1 : 0} onClick={() => { if (!codeHost.accountBusy) codeHost.disconnectAccount(account) }} style={button(!codeHost.accountBusy, codeHost.disconnectConfirmation === account.id)}><text style={{ fontSize: 9 }}>{codeHost.disconnectConfirmation === account.id ? "Confirm disconnect" : "Disconnect"}</text></div>
    </div></Fragment>)}
    <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
      ...
      <div testId="connect-code-host-account" tabIndex={codeHost.accountName.trim() && codeHost.accessToken.trim() && !codeHost.accountBusy ? 0 : -1} onClick={() => { if (codeHost.accountName.trim() && codeHost.accessToken.trim() && !codeHost.accountBusy) codeHost.connectAccount() }} style={button(Boolean(codeHost.accountName.trim() && codeHost.accessToken.trim() && !codeHost.accountBusy))}><text style={{ fontSize: 9 }}>{codeHost.accountBusy ? "Connecting…" : "Connect"}</text></div>
    </div>
  </div>
}
...
function PullRequestCreation({ codeHost, onClose }: { codeHost: WorkbenchCodeHostState; onClose(): void }) {
  return <div testId="pull-request-create" style={{ display: "flex", flexDirection: "column", gap: 7, padding: 10, backgroundColor: palette.panel, borderBottomWidth: 1, borderColor: palette.border }}>
    <div style={{ display: "flex", gap: 7, alignItems: "center" }}><text style={{ color: palette.text, fontSize: 11, fontWeight: 600, flexGrow: 1 }}>Open pull request</text><div tabIndex={0} onClick={onClose} style={button()}><text style={{ fontSize: 9 }}>Close</text></div></div>
    ...
    <div style={{ display: "flex", alignItems: "center", gap: 8 }}><div testId="pull-request-draft" tabIndex={0} onClick={() => codeHost.setPullRequestDraft(!codeHost.pullRequestDraft)} style={button(true, codeHost.pullRequestDraft)}><text style={{ fontSize: 9 }}>{codeHost.pullRequestDraft ? "Draft ✓" : "Draft"}</text></div><div style={{ flexGrow: 1 }} /><div testId="ensure-pull-request" tabIndex={codeHost.canEnsurePullRequest ? 0 : -1} onClick={() => { if (codeHost.canEnsurePullRequest) codeHost.ensurePullRequest() }} style={button(codeHost.canEnsurePullRequest)}><text style={{ fontSize: 9 }}>{codeHost.mutationBusy ? "Working…" : "Find or create"}</text></div></div>
  </div>
}
...
function RemoteBranch(props: { label: string; remotes: WorkbenchCodeHostState["remotes"]; remoteName: string; setRemoteName(value: string): void; branch: string; setBranch(value: string): void; placeholder: string }) {
  return <div style={{ display: "flex", flexDirection: "column", width: "50%", gap: 5 }}><text style={{ color: palette.textFaint, fontSize: 8 }}>{props.label}</text><div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>{props.remotes.map((remote) => <Fragment key={remote.remoteName}><div tabIndex={0} onClick={() => props.setRemoteName(remote.remoteName)} style={button(true, props.remoteName === remote.remoteName)}><text style={{ fontSize: 8 }}>{remote.remoteName}</text></div></Fragment>)}</div><input value={props.branch} placeholder={props.placeholder} onChange={(event) => props.setBranch(event.value ?? "")} style={{ padding: 6, borderWidth: 1, borderColor: palette.border, borderRadius: 5, color: palette.text, backgroundColor: palette.canvas }} /></div>
}
...
function PullRequestList({ codeHost }: { codeHost: WorkbenchCodeHostState }) {
  return <div style={{ display: "flex", flexDirection: "column", width: 270, minWidth: 210, minHeight: 0, borderRightWidth: 1, borderColor: palette.border }}>
    ...
    {codeHost.hasMorePullRequests && <div testId="load-more-pull-requests" tabIndex={codeHost.loadingMorePullRequests ? -1 : 0} onClick={() => { if (!codeHost.loadingMorePullRequests) codeHost.loadMorePullRequests() }} style={{ padding: 9, cursor: "pointer", borderTopWidth: 1, borderColor: palette.border }}><text style={{ color: palette.textMuted, fontSize: 9 }}>{codeHost.loadingMorePullRequests ? "Loading…" : "Load more"}</text></div>}
  </div>
}
...
function PullRequestDetail({ codeHost, openUrl }: { codeHost: WorkbenchCodeHostState; openUrl(url: string): void }) {
  ...
  return <div testId="pull-request-detail" style={{ display: "flex", flexDirection: "column", flexGrow: 1, minWidth: 0, minHeight: 0, overflow: "scroll", padding: 14, gap: 12 }}>
    <div style={{ display: "flex", alignItems: "center", gap: 8 }}><text style={{ color: palette.text, fontSize: 16, fontWeight: 650, flexGrow: 1 }}>{detail.summary.title}</text><div testId="open-pull-request" tabIndex={0} onClick={() => openUrl(detail.summary.webUrl)} style={button()}><text style={{ fontSize: 9 }}>Open on GitHub</text></div></div>
