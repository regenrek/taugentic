...
export function PullRequestsPanel({ codeHost, openUrl }: { codeHost: WorkbenchCodeHostState; openUrl(url: string): void }) {
  ...
  return <div testId="pull-requests-panel" style={{ display: "flex", flexDirection: "column", width: "100%", height: "100%", minHeight: 0, backgroundColor: palette.canvas }}>
    <div style={{ display: "flex", alignItems: "center", gap: 7, minHeight: 42, paddingLeft: 10, paddingRight: 8, borderBottomWidth: 1, borderColor: palette.border }}>
      <text style={{ color: palette.text, fontSize: fontSize(12), fontWeight: 650 }}>Pull requests</text>
      <div style={{ flexGrow: 1 }} />
      <Pressable testId="refresh-code-host" name="Refresh pull requests" onPress={codeHost.refresh} style={button()}><text style={{ fontSize: fontSize(9) }}>Refresh</text></Pressable>
      <Pressable testId="toggle-code-host-connection" name="Accounts" expanded={showConnection} onPress={() => setShowConnection((current) => !current)} style={button()}><text style={{ fontSize: fontSize(9) }}>Accounts</text></Pressable>
      <Pressable testId="toggle-pull-request-create" name="New pull request" disabled={!codeHost.selectedAccount} expanded={showCreation} onPress={() => setShowCreation((current) => !current)} style={button(Boolean(codeHost.selectedAccount))}><text style={{ fontSize: fontSize(9) }}>New PR</text></Pressable>
    </div>
    {codeHost.error && <div style={{ padding: 9, borderBottomWidth: 1, borderColor: palette.border }}><text testId="code-host-error" style={{ color: "#f08080", fontSize: fontSize(10), userSelect: "text" }}>{codeHost.error}</text></div>}
    ...
    {!!codeHost.selectedAccount && <div style={{ display: "flex", flexWrap: "wrap", gap: 6, padding: 8, borderBottomWidth: 1, borderColor: palette.border }}>
      <text style={{ color: palette.textFaint, fontSize: fontSize(9), padding: 7 }}>REPOSITORY</text>
      {codeHost.repositoryLoading && <text style={{ color: palette.textMuted, fontSize: fontSize(9), padding: 7 }}>Loading remotes…</text>}
      {!codeHost.repositoryLoading && !codeHost.remotes.length && <text style={{ color: palette.textMuted, fontSize: fontSize(9), padding: 7 }}>No supported GitHub remotes.</text>}
      {codeHost.remotes.map((remote) => <Fragment key={remote.remoteName}><Pressable testId={`select-code-host-remote-${remote.remoteName}`} name={`Repository ${remote.remoteName} ${remote.repository.owner}/${remote.repository.name}`} role="radio" selected={codeHost.selectedRemote?.remoteName === remote.remoteName} onPress={() => codeHost.selectRemote(remote)} style={button(true, codeHost.selectedRemote?.remoteName === remote.remoteName)}><text style={{ fontSize: fontSize(9) }}>{remote.remoteName} · {remote.repository.owner}/{remote.repository.name}</text></Pressable></Fragment>)}
    </div>}
...
function Empty({ message }: { message: string }) {
  return <div style={{ display: "flex", alignItems: "center", justifyContent: "center", flexGrow: 1, padding: 20 }}><text style={{ color: palette.textMuted, fontSize: fontSize(11), textAlign: "center" }}>{message}</text></div>
}
...
function AccountConnections({ codeHost }: { codeHost: WorkbenchCodeHostState }) {
  return <div testId="code-host-accounts" style={{ display: "flex", flexDirection: "column", gap: 8, padding: 10, backgroundColor: palette.panel, borderBottomWidth: 1, borderColor: palette.border }}>
    <text style={{ color: palette.text, fontSize: fontSize(11), fontWeight: 600 }}>GitHub connections</text>
    <text style={{ color: palette.textMuted, fontSize: fontSize(9) }}>Each local profile has its own OS-protected token. Tokens are never stored with project data.</text>
    {codeHost.accounts.map((account) => <Fragment key={account.id}><div style={{ display: "flex", alignItems: "center", gap: 8 }}>
      <div style={{ display: "flex", flexDirection: "column", flexGrow: 1 }}><text style={{ color: palette.text, fontSize: fontSize(10) }}>{account.displayName}</text><text style={{ color: palette.textFaint, fontSize: fontSize(9) }}>{account.accountLogin} · {account.host}</text></div>
      <Pressable testId={`disconnect-code-host-account-${account.id}`} name={`${codeHost.disconnectConfirmation === account.id ? "Confirm disconnect" : "Disconnect"} ${account.displayName}`} disabled={codeHost.accountBusy} onPress={() => codeHost.disconnectAccount(account)} style={button(!codeHost.accountBusy, codeHost.disconnectConfirmation === account.id)}><text style={{ fontSize: fontSize(9) }}>{codeHost.disconnectConfirmation === account.id ? "Confirm disconnect" : "Disconnect"}</text></Pressable>
    </div></Fragment>)}
    <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", flexGrow: 1, minWidth: 180 }}>
        ...
        {!!codeHost.accessToken && <text testId="code-host-token-ready" style={{ color: palette.accent, fontSize: fontSize(8) }}>Token ready · characters hidden</text>}
      </div>
      <Pressable testId="connect-code-host-account" name="Connect code host account" disabled={!codeHost.accountName.trim() || !codeHost.accessToken.trim() || codeHost.accountBusy} onPress={codeHost.connectAccount} style={button(Boolean(codeHost.accountName.trim() && codeHost.accessToken.trim() && !codeHost.accountBusy))}><text style={{ fontSize: fontSize(9) }}>{codeHost.accountBusy ? "Connecting…" : "Connect"}</text></Pressable>
    </div>
  </div>
}
...
function PullRequestCreation({ codeHost, onClose }: { codeHost: WorkbenchCodeHostState; onClose(): void }) {
  return <div testId="pull-request-create" style={{ display: "flex", flexDirection: "column", gap: 7, padding: 10, backgroundColor: palette.panel, borderBottomWidth: 1, borderColor: palette.border }}>
    <div style={{ display: "flex", gap: 7, alignItems: "center" }}><text style={{ color: palette.text, fontSize: fontSize(11), fontWeight: 600, flexGrow: 1 }}>Open pull request</text><Pressable name="Close pull request creation" onPress={onClose} style={button()}><text style={{ fontSize: fontSize(9) }}>Close</text></Pressable></div>
    <text style={{ color: palette.textMuted, fontSize: fontSize(9) }}>Choose the head and base explicitly. Taugentic matches the exact repository and branch pair before creating anything.</text>
    ...
    <div style={{ display: "flex", alignItems: "center", gap: 8 }}><Pressable testId="pull-request-draft" name="Draft pull request" role="checkbox" checked={codeHost.pullRequestDraft} onPress={() => codeHost.setPullRequestDraft(!codeHost.pullRequestDraft)} style={button(true, codeHost.pullRequestDraft)}><text style={{ fontSize: fontSize(9) }}>{codeHost.pullRequestDraft ? "Draft ✓" : "Draft"}</text></Pressable><div style={{ flexGrow: 1 }} /><Pressable testId="ensure-pull-request" name="Find or create pull request" disabled={!codeHost.canEnsurePullRequest} onPress={codeHost.ensurePullRequest} style={button(codeHost.canEnsurePullRequest)}><text style={{ fontSize: fontSize(9) }}>{codeHost.mutationBusy ? "Working…" : "Find or create"}</text></Pressable></div>
  </div>
}
...
function PullRequestList({ codeHost }: { codeHost: WorkbenchCodeHostState }) {
  return <div style={{ display: "flex", flexDirection: "column", width: 270, minWidth: 210, minHeight: 0, borderRightWidth: 1, borderColor: palette.border }}>
    {codeHost.pullRequestsLoading && <div style={{ padding: 12 }}><text style={{ color: palette.textMuted, fontSize: fontSize(10) }}>Loading pull requests…</text></div>}
    {!codeHost.pullRequestsLoading && !codeHost.pullRequests.length && <div style={{ padding: 12 }}><text testId="pull-request-empty" style={{ color: palette.textMuted, fontSize: fontSize(10) }}>No open pull requests.</text></div>}
