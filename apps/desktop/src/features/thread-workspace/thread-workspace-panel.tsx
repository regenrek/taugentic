...
export function ThreadWorkspacePanel({ workspace }: { workspace: ThreadWorkspacePanelState }) {
  ...
  return <div testId="thread-workspace-panel" style={{ display: "flex", flexDirection: "column", height: "100%", minHeight: 0, backgroundColor: palette.canvas }}>
    <div style={{ display: "flex", alignItems: "center", minHeight: 40, paddingLeft: 12, paddingRight: 8, borderBottomWidth: 1, borderColor: palette.border }}>
      <text style={{ color: palette.text, fontSize: fontSize(12), fontWeight: 650 }}>Thread workspace</text><div style={{ flexGrow: 1 }} />
      <Pressable testId="refresh-thread-workspace" name="Refresh thread workspace" disabled={workspace.busy} onPress={workspace.refresh} style={actionStyle(!workspace.busy)}><text style={{ fontSize: fontSize(10) }}>Refresh</text></Pressable>
    </div>
    <div style={{ display: "flex", flexDirection: "column", flexGrow: 1, minHeight: 0, overflow: "scroll", padding: 12, gap: 14 }}>
      ...
      {fields.map(([field, label]) => <Fragment key={field}><div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
        <div style={{ display: "flex", alignItems: "center" }}><text style={{ color: palette.text, fontSize: fontSize(12), fontWeight: 600 }}>{label}</text><div style={{ flexGrow: 1 }} />
          <Pressable testId={`save-thread-workspace-${field}`} name={`Save ${label}`} disabled={workspace.busy} onPress={() => workspace.save(field)} style={actionStyle(!workspace.busy)}><text style={{ fontSize: fontSize(10) }}>Save</text></Pressable>
        </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
        <text style={{ color: palette.text, fontSize: fontSize(12), fontWeight: 600 }}>Pinned turns</text>
        ...
        {workspace.projection?.pins.map((pin) => <Fragment key={`${pin.runId}:${pin.cursor.sequence}`}><div testId={`thread-workspace-pin-${pin.runId}-${pin.cursor.sequence}`} style={{ display: "flex", alignItems: "center", gap: 8, padding: 8, borderWidth: 1, borderColor: palette.border, borderRadius: 7, backgroundColor: palette.panel }}>
          <text style={{ color: palette.textMuted, fontSize: fontSize(10), userSelect: "text" }}>{`${pin.runId} · ${pin.cursor.sequence}`}</text><div style={{ flexGrow: 1 }} />
          <Pressable testId={`remove-thread-workspace-pin-${pin.runId}-${pin.cursor.sequence}`} name={`Remove pinned turn ${pin.cursor.sequence}`} disabled={workspace.busy} onPress={() => workspace.removePin(pin.cursor.sequence)} style={actionStyle(!workspace.busy)}><text style={{ fontSize: fontSize(10) }}>Remove</text></Pressable>
        </div></Fragment>)}
      </div>
      <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
        <text style={{ color: palette.text, fontSize: fontSize(12), fontWeight: 600 }}>Work log</text>
...
const muted = { color: palette.textMuted, fontSize: fontSize(12) } as const
const errorText = { color: "#f08080", fontSize: fontSize(11) } as const
