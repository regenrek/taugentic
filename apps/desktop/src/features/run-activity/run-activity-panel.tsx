...
export function RunActivityPanel(props: { activity: ReturnTypeUseRunActivity; inbox?: ApprovalsInboxState }) {
  ...
  return <div testId="run-activity-panel" style={{ display: "flex", flexDirection: "column", height: "100%", minHeight: 0, gap: 10, padding: 12, overflow: "scroll" }}>
    ...
    <div style={sectionStyle()}><text style={headingStyle()}>Runs</text>
      ...
      {state.runs.map((run) => <Fragment key={run.id}><Pressable testId={`run-${run.id}`} name={`Open run ${run.objectivePreview ?? run.id}`} selected={state.selectedRunId === run.id} onPress={() => state.selectRun(run.id)} style={{ ...rowStyle(), backgroundColor: state.selectedRunId === run.id ? palette.panelRaised : palette.panel }}><text style={{ color: palette.text, fontSize: 11 }}>{run.relationship.kind === "root" ? "" : "↳ "}{run.objectivePreview ?? run.id}</text><text style={mutedStyle()}>{run.status}</text></Pressable></Fragment>)}
    </div>
    ...
    {state.switchEligible && <Pressable testId="switch-account-and-resume" name="Switch account and resume" onPress={() => void state.switchAccountAndResume()} style={buttonStyle()}><text>Switch account & resume</text></Pressable>}
