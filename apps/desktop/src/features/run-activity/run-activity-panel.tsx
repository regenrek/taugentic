...
export function RunActivityPanel(props: { activity: ReturnTypeUseRunActivity; inbox?: ApprovalsInboxState; copyText?(text: string): void }) {
  ...
  return <div testId="run-activity-panel" style={{ display: "flex", flexDirection: "column", height: "100%", minHeight: 0, gap: 10, padding: 12, overflow: "hidden" }}>
    <div style={{ display: "flex", alignItems: "center", gap: 8 }}><text style={{ color: palette.text, fontSize: fontSize(15), fontWeight: 650 }}>Run & Activity</text><div style={{ flexGrow: 1 }} /><Pressable testId="refresh-run-activity" name="Refresh run activity" onPress={state.refresh} style={buttonStyle()}><text>Refresh</text></Pressable></div>
...
function TimelineTree(props: { runs: readonly { runId: string; depth: number; status: string }[] }) {
  return <>{props.runs.map((run) => <Fragment key={run.runId}><div testId={`timeline-run-${run.runId}`} style={{ ...rowStyle(), paddingLeft: 7 + run.depth * 14 }}><text style={{ color: palette.text, fontSize: fontSize(11) }}>{run.depth ? "↳ " : ""}{run.runId}</text><text style={mutedStyle()}>{run.status}</text></div></Fragment>)}</>
}
...
function headingStyle() { return { color: palette.text, fontSize: fontSize(12), fontWeight: 600 } }
