...
export function PluginsPanel({ plugins, onClose }: { plugins: PluginsState; onClose(): void }) {
  ...
  return <div testId="plugins-panel" accessibilityRole="dialog" accessibilityName="Plugins" style={{ position: "absolute", right: 18, top: 50, width: 420, maxHeight: 740, overflow: "scroll", display: "flex", flexDirection: "column", gap: 10, padding: 12, backgroundColor: palette.panel, borderWidth: 1, borderColor: palette.border, borderRadius: 8 }}>
    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
      <text style={{ color: palette.text, fontSize: 15, fontWeight: 700 }}>Plugins</text><div style={{ flexGrow: 1 }} />
    ...
    <text style={{ color: palette.textMuted, fontSize: 11 }}>Installed packages are disabled. This screen cannot activate or run Plugin code.</text>
    ...
    {plugins.error && <text testId="plugins-error" accessibilityRole="alert" style={{ color: "#f08080", fontSize: 11 }}>{plugins.error}</text>}
    {plugins.mutationError && <text testId="plugins-mutation-error" accessibilityRole="alert" style={{ color: "#f08080", fontSize: 11 }}>{plugins.mutationError}</text>}
    {chooserError && <text testId="plugin-chooser-error" accessibilityRole="alert" style={{ color: "#f08080", fontSize: 11 }}>{chooserError}</text>}
    {plugins.inspection && <div testId="plugin-inspection" style={{ display: "flex", flexDirection: "column", gap: 7, padding: 9, borderWidth: 1, borderColor: palette.border, borderRadius: 7, backgroundColor: palette.canvas }}>
      ...
      <text testId="plugin-inspection-version" style={{ color: palette.textMuted, fontSize: 11 }}>Version {plugins.inspection.version}</text>
      <text testId="plugin-inspection-digest" style={{ color: palette.textMuted, fontSize: 10 }}>{plugins.inspection.digestSha256}</text>
      <text style={{ color: palette.textFaint, fontSize: 10, fontWeight: 700 }}>GRANT CAPABILITIES</text>
      {!plugins.inspection.requestedCapabilities.length && <text testId="plugin-no-requested-capabilities" style={{ color: palette.textMuted, fontSize: 11 }}>This package requests no capabilities.</text>}
    ...
    <text style={{ color: palette.textFaint, fontSize: 10, fontWeight: 700 }}>INSTALLED</text>
    {plugins.loading && !plugins.installations.length && <text testId="plugins-loading" style={{ color: palette.textMuted, fontSize: 11 }}>Loading Plugins…</text>}
    {!plugins.loading && !plugins.installations.length && <text testId="plugins-empty" style={{ color: palette.textMuted, fontSize: 11 }}>No Plugins installed.</text>}
    {plugins.installations.map((installation) => {
      ...
      <text style={{ color: palette.textMuted, fontSize: 11 }}>Version {installation.version}</text>
      <text testId={`plugin-lifecycle-${identity}`} style={{ color: palette.textMuted, fontSize: 11 }}>{installation.lifecycleState === "disabled" ? "Disabled" : installation.lifecycleState}</text>
      <text style={{ color: palette.textMuted, fontSize: 10 }}>{installation.digestSha256}</text>
