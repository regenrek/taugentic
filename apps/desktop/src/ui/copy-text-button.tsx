...
export function CopyTextButton(props: { text: string; copyText?(text: string): void; testId: string; label?: string }) {
  ...
  return <Pressable testId={props.testId} name={label} onPress={() => props.copyText?.(props.text)} style={{ cursor: "pointer", padding: 6, borderRadius: 5, backgroundColor: palette.panelRaised }}><text style={{ color: palette.textMuted, fontSize: fontSize(10) }}>{label}</text></Pressable>
}
